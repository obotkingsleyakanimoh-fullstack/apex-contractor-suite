...
export function Footer() {
  ...
  return (
    <footer className="site-footer bg-[var(--footer-background)] text-white">
      <div className="container-page border-b border-white/10 py-10 sm:py-14">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Button asChild size="lg" className="w-full rounded-full bg-[var(--color-accent)] px-6 text-[var(--heading-color)] hover:bg-accent/90 sm:w-auto sm:px-7">
            <Link to="/request-quote">Get a solar assessment</Link>
          </Button>
        </div>
      </div>

      <div className="container-page grid grid-cols-1 gap-9 py-10 sm:grid-cols-2 sm:gap-10 sm:py-12 lg:grid-cols-4">
        <div className="lg:col-span-2">
          <Link to="/" className="inline-flex items-center gap-3">
            {company?.logo_url ? (
              <img src={company.logo_url} alt={company.company_name} className="h-9 w-auto max-w-[9rem] object-contain sm:h-10 sm:max-w-[10rem]" loading="lazy" />
            ) : null}
      ...
      <div className="border-t border-white/10">
        <div className="container-page flex flex-col gap-3 py-5 text-xs leading-5 text-white/45 sm:flex-row sm:items-center sm:justify-between">
